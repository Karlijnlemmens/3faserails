...
**Soort opdracht** decides what the book opens with. `st.soort` is `'lichtberekening'` (the default, unchanged output) or `'vergelijking'`; a vergelijking gets the **briefing page "Introductie"** directly behind the cover. That page is a redrawing of the supplied design (`AB Briefing vergelijking`): the measurements in the `BRIEF` table were read out of that document rather than eyeballed, and the disclaimer text lives in `BRIEF_DISCLAIMER`, split into segments so the bold fragments survive line-breaking. The project name comes from the field that was already there; `st.documenten` holds the source documents, one per line, and each becomes its own bold line. Both fall back to the design's own `[projectnaam]` / `[naam documenten]` placeholders when empty, and the grey quote block grows if the document list needs more room.
