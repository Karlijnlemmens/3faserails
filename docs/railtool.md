...
Defined in the second half of the script inside an IIFE (`/* =================== RAILCONFIGURATOR =================== */`, from ~line 1281; `CONFIGURATOR V2` is the matching block in the `<style>`). This is a small CAD-like tool: the user draws a top-down floor-plan shape (drag endpoints, add segments, pick a power feed point), and the tool derives every downstream detail deterministically. **Read `railconfigurator-logica.docx` before changing any of the derivation logic** — it's the design/verification doc for this feature and documents rules that are *not* obvious from the code alone.
